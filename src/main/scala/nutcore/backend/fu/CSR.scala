...
class CSR(implicit val p: NutCoreConfig) extends NutCoreModule with HasCSRConst with Sv39Const{
  ...
  // | sd   |
  // | pad1 |
  // | sxl  | hardlinked to 10 (XLEN = 64)
  // | uxl  | hardlinked to 10 (XLEN = 64)
  // | pad0 |
  // | tsr  |
  // | tw   |
  // | tvm  |
  ...
  val hasInstrAccessFault = Wire(Bool())
  val hasLoadAccessFault = Wire(Bool())
  val hasStoreAccessFault = Wire(Bool())
  val hasStoreAddrMisaligned = Wire(Bool())
  val hasLoadAddrMisaligned = Wire(Bool())

  val exceptionPC = Wire(UInt(64.W))
  ...
  if(EnableOutOfOrderExec){
    ...
    hasInstrPageFault := io.cfIn.exceptionVec(instrPageFault) && io.instrValid
    hasInstrAccessFault := io.cfIn.exceptionVec(instrAccessFault) && io.instrValid
    ...
    imemExceptionAddr := Mux(io.illegalJump.valid,
      io.illegalJump.bits,
      Mux(io.cfIn.crossBoundaryFault,
        Mux(vmEnable, SignExt(io.cfIn.pc + 2.U, XLEN), ZeroExt(io.cfIn.pc + 2.U, XLEN)),
        Mux(vmEnable, SignExt(io.cfIn.pc,       XLEN), ZeroExt(io.cfIn.pc,       XLEN))
      )
    )
    exceptionPC := Mux(io.illegalJump.valid,
      io.illegalJump.bits,
      Mux(vmEnable, SignExt(io.cfIn.pc,       XLEN), ZeroExt(io.cfIn.pc,       XLEN))
    )
  }
  ...
  val intrVecIDU = VecInit.tabulate(12)(i => if (IntPriority.contains(i)) intrVec(i) else false.B).asUInt
  BoringUtils.addSource(intrVecIDU, "intrVecIDU")
  ...
  val raiseIntr = VecInit(IntPriority.map(i => io.cfIn.intrVec(i))).asUInt.orR
  ...
  when (io.instrValid) {
    when (hasLoadAddrMisaligned || hasStoreAddrMisaligned) {
      ...
      val tval = Mux(hasInstrPageFault, imemExceptionAddr, dmemExceptionAddr)
      when (delegS) {
        stval := tval
      }.otherwise {
        mtval := tval
