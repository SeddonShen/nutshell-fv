...
class ROB(implicit val p: NutCoreConfig) extends NutCoreModule with HasInstrType with HasBackendConst with HasRegFileParameter{
  ...
  if (!p.FPGAPlatform) {
    for (i <- 0 until RetireWidth) {
      ...
      difftest_commit.valid    := {if (i == 0) RegNext(retireATerm) else RegNext(retireMultiTerms)}
      difftest_commit.pc       := RegNext(SignExt(decode(ringBufferTail)(i).cf.pc, AddrBits))
      difftest_commit.instr    := RegNext(decode(ringBufferTail)(i).cf.instr)
      difftest_commit.skip     := RegNext(isMMIO(ringBufferTail)(i) && valid(ringBufferTail)(i))
      difftest_commit.isRVC    := RegNext(decode(ringBufferTail)(i).cf.isRVC)
      difftest_commit.rfwen    := RegNext(io.wb(i).rfWen && io.wb(i).rfDest =/= 0.U) // && valid(ringBufferTail)(i) && commited(ringBufferTail)(i)
      difftest_commit.fpwen    := false.B
      // difftest.io.wdata    := RegNext(io.wb(i).rfData)
      difftest_commit.wdest    := RegNext(io.wb(i).rfDest)
      difftest_commit.wpdest   := RegNext(io.wb(i).rfDest)
